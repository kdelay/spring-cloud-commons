...
different `DataSource` implementation if you need it to be refreshed. If you set
`spring.cloud.refresh.never-refreshable` yourself, your value replaces the default, so add
`com.zaxxer.hikari.HikariDataSource` to the list if you still want a `HikariDataSource` to be
