...
|spring.cloud.refresh.extra-refreshable |  | Additional bean names or class names for beans to post process into refresh scope.
|spring.cloud.refresh.never-refreshable | `+++com.zaxxer.hikari.HikariDataSource+++` | Comma separated list of bean names or class names for beans to never be refreshed or rebound. Setting this property replaces the default value.
